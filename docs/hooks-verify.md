...
- **[FLAGGED, not fixed — pre-existing spec-authoring question, out of today's scope] `docs/hooks/copilot.md`'s own "Hook Locations (R4)" table lists `.claude/settings.local.json`, `.claude/settings.json`, `~/.claude/settings.json`** — Claude Code's config paths, not GitHub Copilot's. This looks like it was copy-pasted from a Claude Code table during spec authoring and never corrected. The configure guide was deliberately NOT "completed" to match this table (which would propagate a likely error to end users) — it keeps only the two paths that are actually sensible for Copilot (`.github/hooks/*.json`, plugin `hooks.json`). Fixing the spec itself requires re-opening the sealed VERIFIED/COMPLETE spec via the formal spec-review protocol (steps 1–3), not an ad hoc edit — left for a dedicated pass.
...
**Copilot — input normalization gaps, DONE (OI-3), now load-bearing (see routing-bug fix above):** `adapters/copilot.ts`'s `normalize()` now reads `tool_name ?? toolName`, `tool_input` (object, R3) with `parseToolArgs(toolArgs)` fallback, `tool_use_id` from `raw.tool_use_id`, `tool_response` from `raw.tool_response ?? raw.tool_result ?? raw.toolResult` (all 3 documented shapes), and `hook_event_name` directly when present (else infers as before).

**Minor observed caveat (not fixed, out of scope):** `bootstrap/payload.ts`'s `assembleBootstrapPayload` uses a generic `buildHookPayloadJson` (Claude-shaped, single-copy) call purely as a length proxy for the NFR-0004 10,000-char soft-error check, shared across all IDEs. Copilot's actual merged-shape entries are now roughly 2× that proxy's length (top-level + nested duplicate the same content), so the check under-estimates Copilot's real entry size. Pre-existing imprecision (the same proxy already over/under-estimates for other IDEs' actual shapes); not touched here as it's an `assembleBootstrapPayload` architecture question, not part of Bug 2/the routing fix.
