...
- **OI-5 — ⏳ [TODO, analyze deeper] Multi-entrypoint N-fold API re-export.** `run-hook.ts` imports the adapter API (`readStdin`/`normalize`/`formatOutput`/`detectIDE`/`exitCodeFor`/`stderrMessageFor`) from `../adapter`; `scripts/build-bundles.mjs` aliases `../adapter` → `src/hooks/src/entrypoints/adapter-<ide>.ts` per bundle so each plugin bundle carries ONLY its own IDE's adapter code (bundle isolation, enforced by `bundle-isolation.test.ts`). **Cost:** every one of the 5 slim entrypoints AND `adapter.ts` must re-export the FULL API surface, or that bundle fails to build — so any new adapter method (e.g. `stderrMessage` → `stderrMessageFor`, 2026-07-01) requires editing 6 files in lockstep. A single generic entrypoint that selects one adapter at build time (esbuild `define`/alias) would remove the N-fold duplication while preserving isolation. Analyze the trade-off (build complexity vs. maintenance cost) and record the decision here before implementing. Not a bug — a structural refactor. Evidence: `build-bundles.mjs` `onResolve` alias; `entrypoints/adapter-*.ts` (5 files) + `adapter.ts` all export the same surface.
- **OI-6 — ⏳ [TODO] `runHook` / `executeHook` overlap.** `runHook` (`run-hook.ts`) has **zero production call sites** (`grep "runHook("` in `src/hooks/src` = 0) — it is a test-only seam. Now that `executeHook` is exported (2026-07-01, for report-level assertions), `runHook` is just `executeHook` + writing `report.stderrMessage` to a stderr stream; and that stderr write is duplicated in `runAsCli` (`run-hook.ts:44`) and `runHook` (`run-hook.ts:248`). Decide whether to (a) fold `runHook` into `executeHook` + a tiny stderr-writing helper shared with `runAsCli`, or (b) keep `runHook` as the documented in-process test entry. Not a bug — DRY/clarity cleanup. (Dead `runHook` import in `read-once.ts` already removed 2026-07-01.)
