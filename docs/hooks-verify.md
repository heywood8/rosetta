...
**Status (current, see per-IDE table + Change-phase findings + Actions for detail):** Specs for Copilot, Codex, Claude Code, Cursor, Devin Desktop (`windsurf.md`) are **VERIFIED/COMPLETE**. Devin CLI (`devin-cli.md`) is a **doc-grounded DRAFT, not validated** (by decision — Devin Desktop confirmed not to read it). Change-phase (code/requirements/configure) is **DONE** for Claude Code, Cursor, Copilot, and Codex (Action 4 done 2026-07-01). **Still open:** Windsurf's deny-reason-text delivery channel (protocol confirmed working via stderr; not yet wired into Rosetta's own adapter code) — see Open Items / Actions below. (`SubagentStop` is intentionally documented-only, no mapping needed — see User Decisions.)
...
**Still open, NOT done, deferred to Windsurf's own turn — deny reason text delivery.** The *protocol* is verified to work: `windsurf.md` confirms Cascade delivers a `pre_*` hook's stderr to the agent verbatim on exit 2 (Run 1: "Deny via exit-2 + stderr CONFIRMED"; that test wrote the reason to stderr directly). But **Rosetta's own shipped code does not use that channel yet** — `adapters/windsurf.ts`'s `formatOutput` puts the deny reason into `additionalContext`, which `run-hook.ts` then JSON-serializes to **stdout** (`stdout.write(outputText)` — no stderr write exists on the non-error path). Windsurf never parses stdout (confirmed Run 1: "9× exit 0 with textLen 0/stderrLen 0"), so today a real Rosetta-generated Windsurf hook blocks the action (exit 2 works) but the agent never sees why. Fix: route the deny reason through `process.stderr`/the `stderrMessage` path instead of stdout for Windsurf.
