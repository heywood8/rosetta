...
- **Task C — Clean Architecture: identity-switch elimination (2026-06-11)** — Eliminated all 5 identity-switch violations (FR-ARCH-0005) in the TypeScript plugin-generator. (1) Deleted `ModelVocabulary.kind` and `PluginSpec.hookEntryShape` identity-discriminant fields from `src/types.ts` and all 6 spec definitions in `src/spec/targets.ts`. (2) Split `fileNormalizeModels` switch dispatcher into 4 per-vocabulary processors (`fileNormalizeClaudeModels`, `fileNormalizeCursorModels`, `fileNormalizeCopilotModels`, `fileNormalizeCodexModels`) sharing 4 low-level helpers exported from `file-normalize-models.ts` (FR-ARCH-0046). (3) Replaced `pluginAssembleBootstrap` with callback-driven `assembleBootstrapPayload(p, buildEntry, buildRootEntry)` and 4 per-IDE assembler files, each writing `templateContext['bootstrap_hooks']` (one fixed key, FR-VAR-0070, FR-ARCH-0055). (4) Added full cursor bootstrap support via `buildCursorBootstrapEntry`, `buildCursorHookPayloadJson`, and `CURSOR_PLUGIN_ROOT_ENTRY` — fixes prior silent `default: return null` cursor drop (FR-HOOK-0007). (5) Renamed template placeholder keys in 3 `.tmpl` files from `bootstrap_hooks_<ide>` to `bootstrap_hooks`. Multi-vendor model ordering design documented: frontmatter model order is intentional, allowing maintainers to select different providers per agent/skill. Result: tsc clean, 410 tests pass, r2/r3 parity verified. Spec: `plans/plugin-generator/CLEAN-ARCHITECTURE-SPECS.md`.
